...
class TruliaSpider(scrapy.Spider):
    ...
    custom_settings = {'FEED_URI': os.path.join(os.path.dirname(closest_scrapy_cfg()), 'data/iterate/data_sold_%(start)s_%(stop)s_%(time)s.jl'), 
                       'FEED_FORMAT': 'jsonlines'}

    def __init__(self, state = 'IL', city = 'Chicago', start = 1, stop = 10, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.state = state
        self.city = city
        self.start = start
        self.stop = stop
        self.start_urls = ['https://trulia.com/sold/{city},{state}/'.format(state=state, city=city)]
